...
class PerspectiveTransform : public ImageTransformation//继承
{
public:
	...
	virtual void transform(float t, const cv::Mat& source, cv::Mat& result) const;//AlgorithmEstimation.cpp里对原图像变换的主要一步
    
    virtual cv::Mat getHomography(float t, const cv::Mat& source) const;//得到透视变换矩阵
